...
async function getParams(param, parentName = "")
{
	...
	if (param.baseType == 'array') {
		...
		return await asyncReduce(param.components.map(component => () => getParams(component, `${name}.${component.name}`)));
	} else {
		const { value } = await prompts({ ...TYPES[param.baseType], name: 'value', message: `${name} (${param.baseType})` });
...
(async () => {
	...
	const params = await asyncReduce(fragments.deploy.inputs.map(fragment => () => getParams(fragment)));
